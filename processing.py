...
def get_coordinates(input_image, params, model, model_params):
    ...
    for scale in multiplier:
        ...
        output_blobs = model.predict(input_img, verbose=0)
    ...
    for k in range(len(util.hmap_idx)):
        score_mid = paf_avg[:, :, [x - 19 for x in util.hmap_idx[k]]]
        cand_a = all_peaks[util.limb_seq[k][0] - 1]
        cand_b = all_peaks[util.limb_seq[k][1] - 1]
        n_a = len(cand_a)
        n_b = len(cand_b)
        # index_a, index_b = util.limb_seq[k]
        if n_a != 0 and n_b != 0:
    ...
    for k in range(len(util.hmap_idx)):
        if k not in special_k:
            ...
            index_a, index_b = np.array(util.limb_seq[k]) - 1
    ...
    for i in range(18):
        ...
        for s in subset:
            index = s[np.array(util.limb_seq[i]) - 1]
        ...
        limbs.append(np.array(csub))

    people = []
    for limbs_coordinates in zip(*limbs):
        person = {}

        for i, limb_coordinates in enumerate(limbs_coordinates):
            person[COCO_BODY_PARTS[i]] = limb_coordinates
